...
async function upsertDemoTrip(vehicleId: string, driverId: string, createdById: string) {
...
async function upsertDemoMaintenanceLog(vehicleId: string) {
...
async function upsertDemoFuelLog(vehicleId: string, tripId: string) {
...
async function main() {
  ...
  for (const user of demoUsers) {
    ...
    if (!roleId) {
      throw new Error(`Role ${user.role} was not seeded`);
    }
  ...
  if (!tripVehicleId || !tripDriverId || !tripCreatorId) {
    throw new Error("Missing required records for demo trip");
  }

  const trip = await upsertDemoTrip(tripVehicleId, tripDriverId, tripCreatorId);
  const maintenanceVehicleId = vehicleRecords.get("TN09EF9012");

  if (!maintenanceVehicleId) {
    throw new Error("Missing required vehicle for demo maintenance log");
  }

  await upsertDemoMaintenanceLog(maintenanceVehicleId);
  await upsertDemoFuelLog(tripVehicleId, trip.id);
